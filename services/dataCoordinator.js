...
class DataCoordinator {
  ...
  async gatherContextualData(dataNeeds, userLocation, currentTopic, userMessage) {
    ...
    if (dataNeeds.places && userLocation) {
      // 🚀 VERIFICAR SI ES CONSULTA EN TIEMPO REAL/PROXIMIDAD
      if (dataNeeds.realTimeProximity) {
        console.log('🚀 Ejecutando búsqueda optimizada en tiempo real...');
        promises.push(
          (async () => {
            try {
              const result = await realTimeProximityService.executeRealTimeProximitySearch(
                dataNeeds.realTimeProximity,
                userLocation,
                userMessage
              );
              data.places = result.places;
              data.searchMetadata = result.searchMetadata;
              data.optimizedFor = result.optimizedFor;
            } catch (error) {
              console.error('Error en búsqueda tiempo real:', error);
              // Fallback a búsqueda normal
              try {
                const fallbackResult = await this.getPlacesData(userLocation, currentTopic, userMessage);
                data.places = fallbackResult;
              } catch (fallbackError) {
                console.error('Error también en búsqueda fallback:', fallbackError);
                data.places = [];
              }
            }
          })()
